...
void WavetableAudioProcessor::OSCParams::setup (WavetableAudioProcessor& p, int idx)
{
    ...
    bend       = p.addExtParam (id + "bend",       nm + "Bend",        "Bend",      "", { 0.0, 1.0, 0.0, 1.0 }, 0.0, 0.0f);
    oversampling = p.addIntParam (id + "oversampling",     nm + "Oversampling",      "Oversampling",    "", { 1.0, 8.0, 1.0, 1.0 }, 2.0, 0.0f);
...
void WavetableAudioProcessor::FilterParams::setup (WavetableAudioProcessor& p)
{
    ...
    enable           = p.addIntParam (id + "enable",  nm + "Enable",  "",           "", { 0.0, 1.0, 1.0, 1.0 }, 0.0f, 0.0f);
...
void WavetableAudioProcessor::ADSRParams::setup (WavetableAudioProcessor& p)
{
    ...
    velocityTracking = p.addExtParam ("vel",     "Vel",     "Vel",   "", { 0.0, 100.0, 0.0, 1.0 }, 0.0f, 0.0f);
...
{
    fx1         = p.addIntParam ("fxOrder1",   "FX1",       "",   "", { 0.0, 4.0, 1.0, 1.0 }, fxChorus,  0.0f);
    fx2         = p.addIntParam ("fxOrder2",   "FX2",       "",   "", { 0.0, 4.0, 1.0, 1.0 }, fxDistort, 0.0f);
    fx3         = p.addIntParam ("fxOrder3",   "FX3",       "",   "", { 0.0, 4.0, 1.0, 1.0 }, fxDelay,   0.0f);
    fx4         = p.addIntParam ("fxOrder4",   "FX4",       "",   "", { 0.0, 4.0, 1.0, 1.0 }, fxReverb,  0.0f);
...
static gin::ProcessorOptions createProcessorOptions()
{
    ...
    auto opts = gin::ProcessorOptions()
        .withMidiLearn()
        .withoutNewsChecker()
        .withoutUpdateChecker();

    opts.programmingCredits.clear();

    opts.programmingCredits.add (opts.pluginName + " is based on Wavetable by");
    opts.programmingCredits.add("Roland Rabien");
...
{
   {
    ...
    osc1Table = "Analog Classic";
    osc2Table = "Analog Classic";
...
void WavetableAudioProcessor::stateUpdated()
{
    ...
    phaseDistCurves[0].fromBase64Encoding (state.getProperty ("pd1Data").toString());
    phaseDistCurves[1].fromBase64Encoding (state.getProperty ("pd2Data").toString());

    gainCurves[0].fromBase64Encoding (state.getProperty ("pdgain1Data").toString());
    gainCurves[1].fromBase64Encoding (state.getProperty ("pdgain2Data").toString());
...
void WavetableAudioProcessor::updateState()
{
    ...
    state.setProperty ("pd1Data", phaseDistCurves[0].toBase64Encoding(), nullptr);
    state.setProperty ("pd2Data", phaseDistCurves[1].toBase64Encoding(), nullptr);

    state.setProperty ("pdgain1Data", gainCurves[0].toBase64Encoding(), nullptr);
    state.setProperty ("pdgain2Data", gainCurves[1].toBase64Encoding(), nullptr);
}
...
PhaseDistOscillator::Params WavetableAudioProcessor::getLiveWTParams (int osc)
{
    for (auto v : voices)
        if (v->isActive())
