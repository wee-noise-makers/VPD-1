...
void WavetableVoice::noteStarted()
{
    ...
    oscillators[0].setPhaseDistortion (&proc.osc1PD);
    oscillators[1].setPhaseDistortion (&proc.osc2PD);
...
void WavetableVoice::updateParams (int blockSize)
{
    ...
    for (int i = 0; i < Cfg::numOSCs; i++)
    {
        ...
        oscParams[i].oversampling = int (proc.oscParams[i].oversampling->getProcValue());
    }
...
PhaseDistOscillator::Params WavetableVoice::getLiveWTParams (int osc)
{
